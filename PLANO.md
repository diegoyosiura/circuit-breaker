...
> **STATUS (2026-07-09): EXECUTADO INTEGRALMENTE** na branch `refactor/optimizations` — Fases 0, 1 (F1–F8), 2, 3 (R1–R5), **4** e 5 concluídas com todos os gates verdes; resultados no [`CHANGELOG.md`](CHANGELOG.md). A Fase 4 foi implementada como **opt-in inerte por default** (WithBreaker/WithStatusCodeFailure/WithRetryPolicy/WithExponentialBackoff/WithDefaultTimeout + StateReporter + ErrCircuitOpen), com o pacote raiz de aliases e a ressalva do juiz resolvida (tick clampado 25ms → CPU ociosa 0,55%). *Item 4 da Fase 4 (bucket lazy): avaliado e NÃO incorporado nesta rodada — a condição do plano ("só entra se ficar indistinguível em semântica") recomenda ciclo próprio, e o ganho que o motivava (CPU ociosa) já foi obtido pelo tick engrossado.* Validações de não-regressão via workflows multiagente registradas ao final do CB-TESTES.md.
>
