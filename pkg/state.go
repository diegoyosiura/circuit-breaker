...
// classifyOutcome decide o desfecho: erro de transporte (sempre embrulhado
// em *url.Error pelo http.Client) = falha; status >= statusFailMin = falha;
// cancelamento local (ctx/ErrStopped fora de url.Error) = neutro.
func classifyOutcome(resp *http.Response, err error, statusFailMin int) outcome {
	...
	if errors.Is(err, context.Canceled) || errors.Is(err, context.DeadlineExceeded) || errors.Is(err, ErrStopped) {
...
// allow decide a admissão da chamada. Retorna (probe, nil) para admitir —
// probe=true quando a chamada é uma sonda de half-open — ou ErrCircuitOpen.
func (b *breakerState) allow(now time.Time) (bool, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
// report registra o desfecho de uma chamada admitida.
func (b *breakerState) report(probe bool, o outcome) {
	b.mu.Lock()
	defer b.mu.Unlock()
